...
class ApiService {
  ...
  // Stock Data APIs
  async getStocks(shariahOnly = false, includePrices = true, limit = null) {
    const endpoint = shariahOnly ? '/stocks/shariah' : '/stocks/all'
    const params = {
      include_prices: includePrices,
      ...(limit && { limit })
    }
    return this.request(endpoint, { cache: true, params })
  }

  async getStocksFast(limit = 100, includePrices = true) {
    return this.request('/stocks/fast', { 
      cache: true, 
      params: { limit, include_prices: includePrices }
    })
  }
  ...
  async addPosition(positionData) {
    return this.request('/portfolio/positions/add', {
      method: 'POST',
      body: positionData
    })
  }
  ...
  async updatePortfolioFunds(fundsData) {
    return this.request('/portfolio/funds/update', {
      method: 'POST',
      body: fundsData
    })
  }

  async getPortfolioFunds() {
    return this.request('/portfolio/funds', { cache: true })
  }
  ...
  async getPortfolioPositions() {
    return this.request('/portfolio/positions', { cache: true })
  }

  async getPortfolioAllocation() {
    return this.request('/portfolio/allocation', { cache: true })
  }

  async getPortfolioPerformance(period = '30d') {
    return this.request(`/portfolio/performance?period=${period}`, { cache: true })
  }
