#!/usr/bin/env python3
"""
Step 4: Validate Multibagger Predictions
...
class MultibaggerValidator:
    ...
    def calculate_returns_over_time(self, signal, stock_data):
        ...
        try:
            ...
            for i, (date, row) in enumerate(future_data.iterrows()):
                ...
                if current_return > returns_data['max_return']:
                    returns_data['max_return'] = current_return
                    returns_data['max_return_date'] = date.strftime('%Y-%m-%d')
                    returns_data['max_return_price'] = row['Close']
    ...
    def validate_all_signals(self, signals):
        ...
        for i, signal in enumerate(signals, 1):
            ...
            # Print quick result
            print(f"  ✅ Max Return: {returns_data['max_return']:.1f}% ({multibagger_level})")
    ...
    def analyze_results(self, validation_results):
        ...
        analysis = {
            ...
            'multibagger_rate': len(multibaggers) / total_signals * 100,
            'positive_returns_count': len(positive_returns),
            'positive_rate': len(positive_returns) / total_signals * 100,
            'multibagger_breakdown': multibagger_breakdown,
            'performance_stats': {
                'mean_return': np.mean(max_returns),
                'median_return': np.median(max_returns),
                'max_return': np.max(max_returns),
                'min_return': np.min(max_returns),
                'std_return': np.std(max_returns)
            },
            'ml_accuracy': {
                'mean_probability': np.mean(ml_probabilities),
                'median_probability': np.median(ml_probabilities),
                'correlation_prob_return': np.corrcoef(ml_probabilities, max_returns)[0, 1]
            }
        }
...
"""
        ...
        # Show time-based performance for top 5
        report += f"\n📅 TIME-BASED RETURNS (TOP 5 PERFORMERS)\n"
        ...
        for i, result in enumerate(top_performers[:5], 1):
            ...
            for period, return_val in result['returns'].items():
                ...
                report += f"   {label}: {return_val:>7.1f}%"
                if (period == '3_months' or period == '1_year' or 
                    period == '3_years' or period == '6_years'):
                    report += "\n"
                    
        report += f"""

🎯 VALIDATION CONCLUSION
- ML Model Accuracy: {'✅ EXCELLENT' if analysis['multibagger_rate'] >= 60 else '✅ GOOD' if analysis['multibagger_rate'] >= 40 else '⚠️ MODERATE' if analysis['multibagger_rate'] >= 20 else '❌ POOR'}
...
if __name__ == "__main__":
    validator = MultibaggerValidator()
