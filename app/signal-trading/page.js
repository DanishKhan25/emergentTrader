...
export default function SignalTradingPage() {
  ...
  const strategies = [
    ...
    { value: "swing_trading", label: "Swing Trading", icon: Activity },
    ...
    {
      value: "fundamental_growth",
      label: "Fundamental Growth",
      icon: RefreshCw,
    },
    { value: "sector_rotation", label: "Sector rotation", icon: RefreshCw },
    { value: "low_volatility", label: "Low volatility", icon: RefreshCw },
    { value: "pivot_cpr", label: "pivot", icon: RefreshCw },
  ];
  ...
  const processSignalData = (signal) => {
    ...
    const currentPrice =
      ...
      0;
    const entryPrice =
      signal.entry_price || metadata.entry_price || currentPrice;
    ...
    return {
      ...
      shariah_compliant:
        metadata.shariah_compliant || signal.shariah_compliant || false,
      ...
      timestamp: signal.generated_at || signal.created_at,
    };
  };
  ...
  const fetchSignals = async () => {
    try {
      ...
      if (result.success) {
        const rawSignals = result.data?.signals || result.signals || [];
  ...
  const handleBuySignal = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/signals/${selectedSignal.id}/buy`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (result.success) {
        addNotification({
          ...
          message: `Successfully bought ${selectedSignal.symbol}!`,
          duration: 5000,
        });
  ...
  const openBuyModal = (signal) => {
    ...
    setBuyData({
      ...
      target_price:
        signal.target_price?.toString() || (currentPrice * 1.2).toFixed(2),
      stop_loss:
        signal.stop_loss?.toString() || (currentPrice * 0.9).toFixed(2),
    });
        ...
        <CardContent className="p-6">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-100">
            <div className="flex items-center text-sm text-gray-600">
              ...
              {new Date(
                signal.generated_at || signal.timestamp
              ).toLocaleDateString()}
            </div>
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                AI-generated trading opportunities. Click "Buy" to convert
                    ...
                    <p className="text-2xl font-bold text-green-600">
                      {
                        signals.filter(
                          (s) =>
                            (s.confidence || s.confidence_score || 0) >= 0.8
                        ).length
                      }
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-orange-600">
                      {
                        signals.filter((s) => s.strategy === "multibagger")
                          .length
                      }
                    </p>
                  </div>
          ...
          {error ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-6">
                No trading signals found. Generate new signals to get AI-powered
                    ...
                    <span className="font-bold text-blue-600">
                      {formatCurrency(
                        (parseFloat(buyData.quantity) || 0) *
                          (parseFloat(buyData.entry_price) || 0)
                      )}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <p className="capitalize">
                          {selectedSignal.strategy?.replace("_", " ")}
                        </p>
                      </div>
                      <div>
                        <Label className="text-sm font-medium text-gray-600">
                          ...
                          <p className="text-lg font-semibold">
                            {(
                              (selectedSignal.confidence ||
                                selectedSignal.confidence_score ||
                                0) * 100
                            ).toFixed(1)}
                            %
                          </p>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        {selectedSignal.momentum_score && (
                          <div>
                            ...
                            <p className="font-semibold">
                              {selectedSignal.momentum_score.toFixed(2)}
                            </p>
                          </div>
                        )}
                        {selectedSignal.volume_ratio && (
                          <div>
                            ...
                            <p className="font-semibold">
                              {selectedSignal.volume_ratio.toFixed(2)}x
                            </p>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p>
                        {selectedSignal.market_cap
                          ? `₹${(selectedSignal.market_cap / 10000000).toFixed(
                              0
                            )} Cr`
                          : "N/A"}
                      </p>
                    ...
                    <div>
                      ...
                      <p>
                        {selectedSignal.shariah_compliant ? "✅ Yes" : "❌ No"}
                      </p>
                    </div>
                  </div>
                </div>
